...
fifa2$rank_date_year <- format(as.Date(fifa2$rank_date, format = "%Y-%m-%d"), "%Y")
fifa2 %>%
  filter(rank < 6) -> fifa3
f <- ggplot(data = fifa3, aes(x = rank_date, y = rank)) + geom_line(aes(colour = country_abrv))
f
...
makeHistoryRank <- function(conf){
    temp <- data.frame(fifa2 %>%
                         ...
                         group_by(rank_date_year, region) %>%
